...
```{r}
source("report_exercise_9_functions.R") # to access the functions I saved in a separate file called "report_exercise_9_functions"
...
library(lubridate)
library(tidyr)
daily_fluxes <- read_csv("C:/Users/elias/OneDrive/Uni/4. Semester/Geo/AGDS I/agds_report_eliaskoeppel/data/FLX_CH-Dav_FLUXNET2015_FULLSET_DD_1997-2014_1-3.csv") 
...
How would you position the KNN and the linear regression model along the spectrum of the bias-variance trade-off?
    ...
    KNN --> small Bias, big variance
    linear model --> big bias, small variance
...
ggplot(data = df_train_lm, aes(x = TIMESTAMP, y = fitted)) +
  ...
  geom_point(data = df_test_lm, aes(x = TIMESTAMP, y = fitted), color = "red" ) +
  geom_point(data = df_train_knn, aes(x = TIMESTAMP, y = fitted), color = "blue") +
  geom_point(data = df_test_knn, aes(x = TIMESTAMP, y = fitted), color = "black") +
  geom_point(data = df_train_lm, aes(x = TIMESTAMP, y = GPP_NT_VUT_REF), color = "green") +
  theme_classic() +
   scale_color_manual() 
...
# I don't have to split the data again because it is already splitted from the first part of the exxercise. 


# to get models with different k values and in a second step get the MAE values for the test data for each different model --> finally I want to see the developement of the MAE values.



# this part should be replaced with a function (see function get_k_models)
k_models <- list()  
for (i in 1:40){
  k_models[[i]] <- caret::train(
  pp, 
...
get_k_models(daily_fluxes_train, 10) ## Why does this function not work?
...
df <- data.frame(cbind(mae_test, mae_train))
...
plot(df$row_number, df$mae_test)

plot(df$row_number, df$mae_train)




# Create scatter plot with two y values - DOESNE'T WORK!! 
ggplot(data=df, aes(x = row_number) +
  geom_point(aes(y = mae_test), color = "blue") +
  geom_point(aes(y = mae_train), color = "red") +
  xlab("number of k") +
  ylab("mae values"))



# Here I get the same error!!! Whyyy?!
ggplot(data=df, aes(x = row_number, y = mae_test)) +
  geom_point(color = "red") + geom_point(data = df, aes(x = row_number, y = mae_train), color = "blue")




# Die Idee hier ist jetzt, dass wir die Modelle, die wir in k_models gespeichert haben (sie wurden ja f?r den trainings-Datensatz erstellt) nun auf den Test-Datensatz angewendet werden. Dann wollen wir die MAE Werte rausgeben. Und diese dann mit den MAE-Werte f?r den Trainingsdatensatz vergleichen. Diese nnebeneinander in ein Dataframe packen. Und dass dann plotten. So sollte mann auch sehen, f?r welches k der MAE f?r die Test-Daten am kleinsten ist. DAs ist dann der optimale k Wert. 
